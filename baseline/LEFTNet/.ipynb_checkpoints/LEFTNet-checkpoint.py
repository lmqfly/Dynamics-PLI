...
class LEFTNet(torch.nn.Module):
    ...
    def __init__(
        ...
        # layers
        self.edge_embedding = nn.Embedding(4, num_radial)
        self.ele_embedding = nn.Embedding(100,hidden_channels)
        self.res_embedding = nn.Embedding(100,hidden_channels)
        self.pos_embedding = nn.Embedding(100,hidden_channels)
    ...
    def forward(self, data):
        ...
        # embed z
        z_emb = self.ele_embedding(z) + self.res_embedding(z_res)
        ...
        radial_emb = self.radial_emb(dist) + + self.edge_embedding(edge_attr)
