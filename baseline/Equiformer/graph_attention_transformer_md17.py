...
class GraphAttentionTransformerMD17(torch.nn.Module):
    def __init__(self,
        ...
        self.edge_embedding = nn.Embedding(4, self.number_of_basis)
        # layers
        self.ele_embedding = NodeEmbeddingNetwork(self.irreps_node_embedding, 100)
        self.res_embedding = NodeEmbeddingNetwork(self.irreps_node_embedding, 100)
    ...
    def forward(self, data):
        ...
        atom_embedding = self.ele_embedding(z)[0] + self.res_embedding(z_res)[0]
        edge_length = edge_vec.norm(dim=1)
        edge_length_embedding = self.rbf(edge_length) + self.edge_embedding(edge_attr)
        ...
        new_y = self.scale_scatter(outputs, batch, dim=0)
...
def graph_attention_transformer_nonlinear_exp_l2_md17(irreps_in, radius, num_basis=128, 
    ...
    model = GraphAttentionTransformerMD17(
        irreps_in=irreps_in,
        irreps_node_embedding='64x0e+32x1e+16x2e', num_layers=6,
        irreps_node_attr='1x0e', irreps_sh='1x0e+1x1e+1x2e',
        max_radius=radius,
